...
# -------------------- NOTA IMPORTANTE --------------------
# El Script Filtra Automáticamente Algunos Caracteres Especiales Que En Determinadas Voces IVONA
# Pueden Provocar WAVS Inválidos De 47 Bytes O Errores De Síntesis.
# Si Con Algún Texto Te Falla, Fíjate En Qué Bloque Ha Sucedido. Si Es Un Caracter Raro, Solo
# Tienes Que Añadirlo Aqui: texto = texto.translate(str.maketrans('', '', '()[]<>{}"\'“”‘’«»'))
# Busca Esa Línea En El Código Y Añade El Nuevo Caracter Problemático.
...
def fuego_m8ax():
    ...
    ancho = 110
    alto = 32
    texto = "M 8 A X     X T T S     E N G I N E   -   I V O N A  C O N C H I T A  T O D O S  L O S  B L O Q U E S"
...
def aviso(inicio):
    ...
    while not stop_event.is_set():
        ...
        if contador % 2 == 0:
            ...
            print(
                f"\n{color}M8AX ... ESPERA ... Generando Audio Por Partes ... Usando Ivona Conchita ... Puede Tardar Bastante Según CPU O GPU ... M8AX{reset}\n",
                flush=True,
            )
...
print(f"- Generando Audio Con Muestra(s) ➤ Ivona Conchita\n")
print(f"- Usando 1 Muestra(s) De Voz\n")
...
_TTS_WORKER_SCRIPT = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "_m8ax_tts_worker.py"
)

with open(_TTS_WORKER_SCRIPT, "w", encoding="utf-8") as _f:
    _f.write(
        "import sys, os, gc\n"
        ...
        "texto = sys.argv[1]\n"
        "salida = sys.argv[2]\n"
        "voz_nombre = sys.argv[3]\n"
        "rate = int(sys.argv[4])\n"
        "pythoncom.CoInitialize()\n"
        "tts = win32com.client.Dispatch('SAPI.SpVoice')\n"
        "stream = win32com.client.Dispatch('SAPI.SpFileStream')\n"
        "stream.Open(salida, 3)\n"
        "tts.AudioOutputStream = stream\n"
        "tts.Rate = rate\n"
        "for v in tts.GetVoices():\n"
        "    if voz_nombre in v.GetDescription():\n"
        "        tts.Voice = v\n"
        "        break\n"
        "tts.Speak(texto, 0)\n"
        "stream.Close()\n"
...
def tts_ivona_subprocess(texto, salida, voz_nombre="Conchita", rate=0, timeout=120):
    resultado = subprocess.run(
        [sys.executable, _TTS_WORKER_SCRIPT, texto, salida, voz_nombre, str(rate)],
        timeout=timeout,
        stderr=subprocess.DEVNULL,
    )
    if resultado.returncode != 0:
        raise RuntimeError(f"TTS worker returncode ➤ {resultado.returncode}")
...
for i, bloque in enumerate(bloques, 1):
    ...
    if i % 500 == 0 and i > 0:
        print(
            f"\n🔄 Bloque {i:06d} ➤ Pausa De Seguridad Para Liberar COM De Ivona...\n",
            flush=True,
        )
        time.sleep(15)
        print(f"🔄 Reanudando...\n", flush=True)
    ...
    while True:
        ...
        try:
            tts_ivona_subprocess(bloque, nombre_salida, "Conchita", 0, timeout=120)
            fin_bloque = time.time()
        except Exception as e:
            print(f"\n- Error En Bloque {i:06d} Intento {intento} ➤ {e}")
            print(f"\n  ↺ Matando Worker Y Reintentando...\n", flush=True)
            try:
                ps_cmd = (
                    "Get-WmiObject Win32_Process | "
                    "Where-Object { $_.Name -like '*python*' -and "
                    "$_.CommandLine -like '*_m8ax_tts_worker*' } | "
        ...
        if not os.path.exists(nombre_salida):
            print(f"\n- Bloque {i:06d} Intento {intento} ➤ WAV No Generado")
            print(f"\n  ↺ Reintentando En 0.5 Segundos...\n", flush=True)
            time.sleep(0.5)
            continue

        tam_wav = os.path.getsize(nombre_salida)

        if tam_wav < TAMANO_MINIMO_WAV:

            print(
                f"\n- Bloque {i:06d} Intento {intento} ➤ WAV Sospechoso ( {tam_wav} Bytes < {TAMANO_MINIMO_WAV} )"
            )

            print(f"\n  ↺ Matando Worker Colgado Y Reintentando...\n", flush=True)
    ...
    if not bloque_ok:
        print(f"\n- Bloque {i:06d} ➤ Saltado ( No Debería Ocurrir )\n\n")
    ...
    print(
        ...
        f"\033[38;2;0;255;120m > Voz ➤ Ivona Conchita\033[0m\n"
...
print(
    f"- Voces Distintas Utilizadas ➤ 1 - ( Número Real De Voces Diferentes Usadas En Todo El Audio )\n"
)
    ...
    def actualizar_narrador():
        ...
        while res_video.poll() is None:
            ...
            if inicio_n <= tiempo_actual <= fin_n:
                ...
                nombre_n = "Ivona Conchita"

                texto_narrador = f"--- Narrador Ivona Conchita ---"
...
if (
    ...
    with open(os.devnull, "w") as fnull:
        ...
        with contextlib.redirect_stdout(fnull):
            ...
            try:
                tts_ivona_subprocess(
                    mensaje_audio_final, "M8AX_Final.wav", "Conchita", 0, timeout=120
                )
            except Exception:
                pass
...
if os.path.exists(_TTS_WORKER_SCRIPT):
    os.remove(_TTS_WORKER_SCRIPT)
